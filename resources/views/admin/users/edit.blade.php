...
<div class="row justify-content-center py-4">
    <div class="col-md-8">
        <div class="d-flex align-items-center mb-4">
            ...
            <h2 class="text-white text-uppercase fw-bold mb-0" style="letter-spacing: 1px;">Sửa Hồ Sơ: <span style="color: var(--gold);">{{ $user->name }}</span></h2>
        </div>
            ...
            <div class="card-body">
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" class="form-control form-control-lg luxury-input @error('name') is-invalid @enderror" value="{{ old('name', $user->name) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" name="email" class="form-control form-control-lg luxury-input @error('email') is-invalid @enderror" value="{{ old('email', $user->email) }}">
                        @error('email') <div class="invalid-feedback text-danger">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-bold text-white">Mật khẩu mới (Bỏ trống nếu không muốn đổi)</label>
                        <input type="password" name="password" class="form-control form-control-lg luxury-input @error('password') is-invalid @enderror" placeholder="••••••••">
                    ...
                    <div class="row mb-5">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" class="form-select luxury-select">
                                ...
                                <option value="0" {{ old('status', $user->status) == '0' ? 'selected' : '' }}>Đóng băng tài khoản</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-gold btn-lg px-5 rounded-pill">
                            <i class="fa-solid fa-arrows-rotate me-2"></i> Cập Nhật Hồ Sơ
                        </button>
                    </div>
                </form>
            </div>
